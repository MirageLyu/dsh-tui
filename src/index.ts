...
// Type import declaration-merges the `approval` service onto `Context`; the
// approval answerer queue is registered by ./chat/approval.
import type {} from '@deepseek-ai/dsh-user-approval'
// Type import declaration-merges the `permissionPresets` service onto `Context`.
...
export const inject = ['agents', 'sessions', 'commands', 'userQuestions', 'approval', 'tools', 'llm', 'systemPrompt', 'tokenMeter', 'tuiPrompt']
...
export function createTuiChat(
  ...
  // chat.children index where the open step's leading messages insert. The
  // loop appends step/start BEFORE the step's user/message events, so the
  // assistant header is already attached when they arrive; transcript order
  // (user blocks, then the assistant step) therefore inserts them ahead of
  // the header instead of appending at the tail.
  let leadingMessageCursor: number | undefined
  ...
  /** The most recently settled assistant step — Tab folds/unfolds its thinking. */
  let lastAssistantStep: StreamingAssistantComponent | undefined
  /** The most recently rendered tool card — Ctrl+Enter opens its full-screen detail. */
  let lastToolCard: ToolCardComponent | undefined
  ...
  const [
    cwdValue, gitValue, tokenValue, permissionValue, planValue,
    modelValue, contextValue, queuedValue, symbolValue, indicatorValue,
  ] = promptValues
  /* v8 ignore next -- the fixed built-in registration list always supplies each handle. */
  if (cwdValue === undefined || gitValue === undefined || tokenValue === undefined || permissionValue === undefined
    || planValue === undefined || modelValue === undefined || contextValue === undefined || queuedValue === undefined
  ...
  const updatePromptValues = (): void => {
    ...
    permissionValue.set(preset === undefined || preset === 'custom' ? undefined : `  ${palette.dim(displayText(preset))}`)
    planValue.set(foldPlanActive(agent.session.events) ? `  ${palette.dim('⧉ plan')}` : undefined)
  ...
  const clearStreaming = (): void => {
    ...
    streaming = undefined
    leadingMessageCursor = undefined
  }
  ...
  const startAssistantStep = (position: StepPosition): void => {
    ...
    leadingMessageCursor = chat.children.indexOf(streaming)
  }

  /** Insert one leading block of the open step ahead of its assistant header, or append at the tail between steps. */
  const insertLeadingMessage = (component: Component): void => {
    if (leadingMessageCursor === undefined) {
      chat.addChild(component)
      return
    }
    chat.children.splice(leadingMessageCursor, 0, component)
    leadingMessageCursor += 1
  }
  ...
  const renderEvent = (
    ...
    switch (event.type) {
      case 'user/message': {
        ...
        if (source.kind !== 'user') {
          ...
          if (references !== undefined) {
            insertLeadingMessage(new Spacer(1))
            insertLeadingMessage(new Text(palette.dim(`Referenced sessions · ${references.map(displayText).join(', ')}`), 0, 0))
            break
          }
        ...
        if (text) {
          insertLeadingMessage(new Spacer(1))
          insertLeadingMessage(new UserMessageComponent(text, palette, mdTheme))
      ...
      case 'tool/call':
        lastToolCard = parsedTool(event)
        chat.addChild(lastToolCard)
      ...
      case 'step/end':
        ...
        if (streaming !== undefined) lastAssistantStep = streaming
        streaming = undefined
        leadingMessageCursor = undefined
        break
      ...
      case 'turn/end': {
        ...
        switch (reason.kind) {
          case 'completed':
            break
          case 'error': {
            // `agent/error` already reported the live failure with its full
            // cause chain; the turn marker repeats it only when no live error
            // of this turn reached the channel.
            const turn = event.data.turn
            if (![...liveErrors].some(key => key.startsWith(`${turn}:`))) {
              appendNotice(reason.error.message, 'error')
            }
            for (const key of liveErrors) {
              if (key.startsWith(`${turn}:`)) liveErrors.delete(key)
            }
            break
          }
          case 'aborted': {
            const cause = reason.reason
            if (cause.kind === 'user') appendNotice('Turn cancelled.', 'warning')
            else if (cause.kind === 'disposed') appendNotice('Turn stopped: the agent was disposed.', 'warning')
            else if (cause.kind === 'parent') appendNotice('Turn cancelled by its parent agent.', 'warning')
            else if (cause.kind === 'hook') appendNotice(`Turn cancelled by a hook: ${cause.reason}`, 'warning')
            else appendNotice('Turn cancelled.', 'warning')
            break
          }
          case 'blocked':
            appendNotice('Turn blocked.', 'warning')
            break
          case 'max-tokens':
  ...
  const rebuildTranscript = (populateHistory: boolean): void => {
    leadingMessageCursor = undefined
    lastAssistantStep = undefined
    lastToolCard = undefined
    chat.clear()
    toolCards.clear()
    allToolCards.clear()
    contextCards.clear()
  ...
  /** The scheme the terminal last reported; undefined until the startup query answers. */
  let detectedScheme: TerminalColorScheme | undefined
  // A configured scheme pins the palette before the terminal query; `auto`
  // (the default) follows the detected one, falling back to dark.
  if (resolved.theme.scheme !== undefined) applyColorScheme(resolved.theme.scheme)
  ...
  const runTheme = (rawInput: string): CommandResult => {
    ...
    if (value === '') {
      return {
        kind: 'success',
        text: `current scheme ${currentScheme}${detectedScheme === undefined ? '' : ` (terminal reports ${detectedScheme})`}`,
      }
    }
    if (value === 'dark' || value === 'light') {
      applyColorScheme(value)
      return { kind: 'success', text: `scheme ${value}` }
    }
    if (value === 'auto') {
      applyColorScheme(detectedScheme ?? 'dark')
      return { kind: 'success', text: `scheme auto (${detectedScheme ?? 'dark'})` }
    }
    return { kind: 'error', text: `Unknown /theme argument "${value}". Usage: /theme [dark|light|auto]` }
  }
  ...
  const disposeSchemeListener = ui.onTerminalColorSchemeChange((scheme) => {
    ...
    // A configured scheme pins the palette; auto follows the terminal report.
    if (resolved.theme.scheme === undefined) applyColorScheme(scheme)
  })
  ...
  /** Open the most recent tool card's complete body in a full-screen pager. */
  let pagerOverlay: TuiOverlaySession | undefined
  const openToolCardPager = (): void => {
    void pagerOverlay?.close()
    if (lastToolCard === undefined) {
      appendNotice('No tool call in this transcript yet.', 'info')
      return
    }
    const card = lastToolCard
    const session = overlayManager.open({
      create: () => new FullScreenPager(
        () => card.detailLines(runtime.terminal.columns - 2),
        () => runtime.terminal.rows,
        palette,
        () => { void session.close() },
      ),
      options: { width: '100%', maxHeight: '100%', margin: 0 },
    })
    pagerOverlay = session
    void session.closed.then(() => {
      if (pagerOverlay === session) pagerOverlay = undefined
    })
    requestRender()
  }
  ...
  const showHelp = (): void => {
    ...
    chat.addChild(new Text([
      ...
      'Ctrl+T fold thinking • Ctrl+F full-screen tool detail • Ctrl+C cancel; clear input or exit while idle • Ctrl+D exit',
      '',
      ...commandLines,
  ...
  const showStatus = async (signal: AbortSignal): Promise<void> => {
    ...
    const groups: readonly (readonly StatusCardRow[])[] = [
      ...
      [
        ['Permission', displayText((ctx.get('permissionPresets') as
          | { current(events: readonly SessionEvent[]): string }
          | undefined)?.current(agent.session.events) ?? 'unset')],
        ['Plan mode', foldPlanActive(agent.session.events) ? 'active' : 'inactive'],
      ],
      [
        ['Agent', [
          agent.status,
  ...
  const commandFiber = agent.ctx.inject(['commands'], (commandCtx) => {
    ...
    commandCtx.commands.register({
      name: 'theme',
      description: 'Switch the light/dark color scheme (bare shows the current one)',
      input: { hint: '[dark|light|auto]' },
  ...
  const dispatchMessage = (content: ContentBlock[], attachedContext?: UserMessage): void => {
    ...
    const detachSubmit = ctx.on('agent/pre-step', async (payload, next) => {
      if (payload.agent !== agent || !payload.messages.some(entry => entry.id === message.id)) return next()
      cleanup()
    ...
    const detachDiscard = ctx.on('agent/inbox/discarded', (payload) => {
      if (payload.agent !== agent) return
      discarded.add(payload.message.id)
  ...
  const removeInputListener = ui.addInputListener((data) => {
    ...
    if (matchesKey(data, Key.ctrl('t'))) {
      // Fold/unfold the most recent assistant step's thinking inline.
      if (lastAssistantStep !== undefined && lastAssistantStep.hasThinking()) {
        lastAssistantStep.toggleThinking()
        requestRender()
      } else {
        appendNotice('No thinking block to fold in the latest assistant step.', 'info')
      }
      return { consume: true }
    }
  ...
  const disposeDequeued = ctx.on('agent/inbox/claimed', (payload) => {
  ...
  const disposeDiscarded = ctx.on('agent/inbox/discarded', (payload) => {
    if (payload.agent !== agent) return
    if (pendingSteering.delete(payload.message.id)) refreshStatus()
  })
  ...
  const disposeError = ctx.on('agent/error', (payload) => {
    if (payload.agent !== agent) return
    liveErrors.add(`${payload.turn}:${payload.step}`)
...
/** One mounted terminal channel and its lifecycle controller. */
 ...
 * @param runtime - Terminal and process-exit boundary.
 * @returns the mounted channel when the agent already exists; `undefined`
 *   while the channel waits for a config-created agent (the runner path
 *   always creates the agent first, so it always receives a channel).
 */
export function mountTui(ctx: Context, config: Config, runtime: TuiRuntime): MountedTui | undefined {
  const sessionId = SessionId(config.sessionId ?? 'main')
  ...
  const start = (agent: Agent): MountedTui | undefined => {
    if (settled || !matchesConfiguredIdentity(agent)) return undefined
    settled = true
    stopWaiting()
    let controller: TuiController | undefined
    ctx.effect(() => {
      controller = createTuiChat(ctx, config, runtime)
      return () => { void controller?.dispose() }
    }, 'ui-tui')
    /* v8 ignore next -- the effect installs the channel synchronously. */
    if (controller === undefined) throw new Error('ui-tui: channel did not mount')
    return { controller }
  }
  ...
  const disposeFailure = ctx.on('agent-loop/config-start-failed', (payload) => { fail(payload.sessionId, payload.error) })
  const existing = ctx.agents.roots().find(agent => agent.id === sessionId)
  if (existing !== undefined) return start(existing)
  return undefined
}
...
/** Whether the session log's last `plan/mode` event left plan mode active. */
function foldPlanActive(events: readonly SessionEvent[]): boolean {
  for (let index = events.length - 1; index >= 0; index -= 1) {
    const event = events[index] as unknown as { type?: string; data?: { active?: boolean } }
    if (event.type === 'plan/mode') return event.data?.active === true
  }
  return false
}

/** Default model the runner starts from when neither --model nor a stored selection names one. */
interface DefaultModelSelection {
  currentSelection(): { provider: string; model: string }
}

/** The `--model` flag split into its provider and model halves. */
...
/**
 * Parse a `--model` value: `provider/model` names both, a bare value names
 * only the model and keeps the default provider.
 * @param raw - the flag value.
 * @returns the override halves.
 */
...
/**
 * Strip this invocation's resume selection so a handoff child starts on the
 * session it is handed, without duplicating a stale flag.
 * @param argv - the launcher arguments after the script path.
 * @returns the same arguments with `--resume <id>` and `--continue` removed.
 */
function stripResumeFlags(argv: readonly string[]): string[] {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (token === '--continue') continue
    kept.push(token as string)
  }
  return kept
}

/** Runner-owned state for one TUI process: the live agent handle and mounted channel. */
interface TuiRunnerState {
  handle: AgentHandle | undefined
  mounted: MountedTui | undefined
}

/**
 * Build the process runner: create or resume the agent for one invocation,
 * mount the channel over it, and re-exec the process for a cross-workspace
 * in-place resume handoff.
 * @param ctx - booted tree carrying the agent registry and optional startup values.
 * @param config - validated TUI presentation configuration.
 * @param startup - parsed launcher flags, absent for embedders without a launcher.
 * @returns the runner used by {@link apply}.
 */
function createTuiRunner(ctx: Context, config: Config, startup: TuiStartupValues | undefined): {
  start(runtime: TuiRuntime): Promise<void>
  handoff(sessionId: SessionId, cwd: string): Promise<never>
} {
  const state: TuiRunnerState = { handle: undefined, mounted: undefined }

  /** Agent creation options resolved from --model, then the stored default selection. */
  const resolveAgentOptions = (): AgentOptions => {
    if (startup?.model !== undefined) return parseModelOverride(startup.model)
    const selection = ctx.get('agentDefaultModel') as DefaultModelSelection | undefined
    const current = selection?.currentSelection()
    if (current === undefined) return {}
    return { provider: current.provider, model: current.model }
  }

  const mount = (sessionId: SessionId, runtime: TuiRuntime): MountedTui => {
    const mounted = mountTui(ctx, Object.assign({}, config, { sessionId }), runtime)
    if (mounted === undefined) {
      throw new Error(`ui-tui: session "${sessionId}" is not registered after creation`)
    }
  ...
  return {
    async start(runtime: TuiRuntime): Promise<void> {
      // Loader siblings mount concurrently. Await the complete application
      // before creating the agent so its scoped tools are not half-composed.
      await ctx.get('loader')?.await()
      const agents = ctx.get('agents')
      if (agents === undefined) throw new Error('ui-tui: the agent registry is not mounted')
      let resumeId: SessionId | undefined
      if (startup?.resumeSessionId !== undefined) {
        resumeId = SessionId(startup.resumeSessionId)
      } else if (startup?.continueMostRecent === true) {
        const persistence = ctx.get('sessionPersistence')
        if (persistence === undefined) {
          throw new Error('ui-tui: --continue needs a mounted session-persistence backend')
        }
        const headers = await persistence.list()
        const mostRecent = headers.reduce<SessionHeader | undefined>(
          (best, header) => best === undefined || header.createdAt > best.createdAt ? header : best,
          undefined,
        )
        if (mostRecent === undefined) throw new Error('ui-tui: --continue found no persisted sessions')
      ...
      const handle = resumeId === undefined
        ? await agents.create({
          ...
          meta: { cwd: startup?.workspace ?? process.cwd() },
          agentOptions,
        })
        : await agents.resume({ resumeSessionId: resumeId, agentOptions })
      state.handle = handle
      mount(handle.agent.id, runtime)
    },

    async handoff(sessionId: SessionId, cwd: string): Promise<never> {
      const handle = state.handle
      if (handle === undefined) throw new Error('ui-tui: no live session to hand off from')
      // Release the terminal and tear down the channel BEFORE the child boots,
      // so the two processes never race over raw mode.
      if (state.mounted !== undefined) await state.mounted.controller.dispose()
      state.mounted = undefined
      // The child re-execs into the selected session's own workspace: process
      // cwd, not the restored header, is what filesystem and shell tools
      // resolve against.
      const script = process.argv[1]
      if (script === undefined) throw new Error('ui-tui: cannot hand off resume without a script path')
      spawn(process.execPath, [script, ...stripResumeFlags(process.argv.slice(2)), '--resume', sessionId], {
        cwd,
        stdio: 'inherit',
        env: process.env,
      })
      ...
      else disposeRootAndExit(ctx, 0)
      // The child owns the terminal from here; success never returns.
      await new Promise<never>(() => {})
      /* v8 ignore next -- unreachable: the parent exits above. */
      throw new Error('ui-tui: resume handoff returned without exiting')
    },
  }
}
...
export function apply(ctx: Context, config: Config): void {
  if (!process.stdin.isTTY || !process.stdout.isTTY) {
    throw new Error('ui-tui: both stdin and stdout must be TTYs; use the one-shot headless profile for pipes')
  }
  ...
  const runner = createTuiRunner(ctx, Object.assign(
    {},
    config,
  ...
  const runtime: TuiRuntime = {
    ...
    exit: (code) => {
      ...
      else disposeRootAndExit(ctx, code)
    },
    handoffResume: (sessionId, cwd) => runner.handoff(sessionId, cwd),
    ...goodbyeMessage === undefined ? {} : { goodbyeMessage },
  }
  void runner.start(runtime).catch((error: unknown) => {
    runtime.terminal.write(displayText(`ui-tui: startup failed: ${errorChain(error)}\n`))
    runtime.exit(1)
  })
}
/* v8 ignore stop */
