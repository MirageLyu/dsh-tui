 ...
 * @module @miragelyu/dsh-tui/config
 */
...
export interface TuiThemeConfig {
  ...
  color?: boolean
  /**
   * Pin the light/dark scheme. Absent, the terminal's reported scheme wins
   * (dark when the terminal does not report one); `/theme` switches a live
...
export interface TuiConfig {
  ...
  /**
   * Collapse a reasoning block into one `✦ Thinking · N words` summary line,
   * expanded inline with Tab on the most recent assistant step. `false`
   * renders reasoning inline whenever `showReasoning` is on.
   */
...
const collapseThinkingSchema = z.boolean().default(true)
...
const DEFAULT_LEFT_PROMPT = '${cwd}${git/worktree}${permission}${plan}${model}${token_meter/cache_hit_rate}${context}'
...
const schemeSchema = z.union(['dark', 'light'] as const)
...
const tuiConfigSchemaFields = {
  ...
  collapseThinking: collapseThinkingSchema,
...
export interface Config extends TuiConfig {
   ...
   * the user typed `/skill:<name>`. Absent leaves the first turn to the user.
   */
  initialSkill?: string
}
...
export function resolveTuiConfig(config: TuiConfig | undefined): ResolvedTuiConfig {
  return {
    ...
    collapseThinking: config?.collapseThinking ?? true,
