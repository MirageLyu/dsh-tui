 ...
 * @module @miragelyu/dsh-tui/components/dialogs
 */
...
/**
 * Inline approval dialog: one permission question with a tool name, the
 * asker's reason, and two keyboard decisions. Enter/`y` grants the single
 * action (`allowed-once`); Escape/`n` rejects it; Ctrl+C withdraws
 * (`cancelled`).
 */
export class ApprovalDialog implements Component, Focusable {
  ...
  invalidate(): void {
    /* The dialog redraws through the owning overlay's invalidate cycle. */
  }
  ...
  handleInput(data: string): void {
    ...
    if (matchesKey(data, Key.enter) || data.toLowerCase() === 'y') {
      this.decided = true
      this.done('allowed-once')
    } else if (matchesKey(data, Key.escape) || data.toLowerCase() === 'n') {
      this.decided = true
      this.done('rejected')
  ...
  render(width: number): string[] {
    const horizontalPadding = Math.min(2, Math.max(0, Math.floor((width - 1) / 2)))
    const innerWidth = Math.max(1, width - horizontalPadding * 2)
    const toolLine = this.palette.bold(this.palette.accent(displayText(this.request.toolName)))
    const titleLines = wrapTextWithAnsi(`Approve this ${toolLine} call?`, innerWidth)
    const reasonLines = this.request.reason === undefined
      ? []
      : wrapTextWithAnsi(this.palette.dim(displayText(this.request.reason)), innerWidth)
    const callLines = this.request.callId === undefined
      ? []
      : wrapTextWithAnsi(this.palette.dim(displayText(String(this.request.callId))), innerWidth)
    const hint = this.palette.dim('Enter/Y allow once · Esc/N reject · Ctrl+C cancel')
    const frame = dialogFrame(this.palette, innerWidth)
    const content = [
      ...titleLines,
...
/** Draw one approval dialog inside a rounded frame. */
function dialogFrame(palette: Palette, innerWidth: number): (content: string[]) => string[] {
  return (content: string[]): string[] => {
    const top = `${palette.dim('╭')}${'─'.repeat(innerWidth + 2)}${palette.dim('╮')}`
    const bottom = `${palette.dim('╰')}${'─'.repeat(innerWidth + 2)}${palette.dim('╯')}`
    const rows = content.map(line => `${palette.dim('│')} ${line}${' '.repeat(Math.max(0, innerWidth - visibleWidth(line)))} ${palette.dim('│')}`)
    return [top, ...rows, bottom]
  }
}

/**
 * Full-screen scrollable detail pager: the complete body of one tool card (or
 * any line source) over the whole viewport, with Claude Code-style page
 * navigation. Opens through Ctrl+Enter on the most recent tool card; Esc or
 * q closes.
 */
export class FullScreenPager implements Component, Focusable {
  focused = false
  private offset = 0

  constructor(
    /** The full body lines, already wrapped and styled, re-derived on open. */
    private readonly lines: () => readonly string[],
    /** Terminal rows available for one page, from the host viewport. */
  ...
  invalidate(): void {
    /* The owning overlay redraws through its own invalidate cycle. */
  }

  handleInput(data: string): void {
    const viewport = this.lines().length
    if (matchesKey(data, Key.escape) || data.toLowerCase() === 'q') {
      this.close()
    } else if (matchesKey(data, Key.down) || matchesKey(data, Key.ctrl('n'))) {
      this.offset = Math.min(this.offset + 1, Math.max(0, viewport - 1))
    } else if (matchesKey(data, Key.up) || matchesKey(data, Key.ctrl('p'))) {
      this.offset = Math.max(0, this.offset - 1)
    } else if (matchesKey(data, Key.pageDown)) {
      this.offset = Math.min(this.offset + this.pageSize(), Math.max(0, viewport - 1))
    } else if (matchesKey(data, Key.pageUp)) {
      this.offset = Math.max(0, this.offset - this.pageSize())
    } else if (matchesKey(data, Key.home)) {
      this.offset = 0
    } else if (matchesKey(data, Key.end)) {
      this.offset = Math.max(0, viewport - 1)
    }
  }
  ...
  render(width: number): string[] {
    ...
    for (let index = 0; index < lines.length; index += 1) {
      ...
      /* v8 ignore next -- the pager wraps its own input against the terminal width. */
      if (line === undefined) continue
      rows.push(line.length > inner ? line.slice(0, inner - 1) + '…' : line)
    }
    const header = this.palette.bold(this.palette.accent('Detail'))
    const hint = this.palette.dim('↑/↓ · PgUp/PgDn · Home/End · Esc/q close')
    const page = this.pageSize()
    return [
      `${header}  ${hint}`,
      this.palette.dim('─'.repeat(inner)),
      ...rows.slice(this.offset, this.offset + page),
    ]
  }

  /** Rows visible per page: the terminal height minus the two header rows. */
  private pageSize(): number {
    return Math.max(4, this.pageRows() - 2)
  }
}
