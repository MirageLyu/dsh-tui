 ...
 * @module @miragelyu/dsh-tui/components/transcript
 */
 ...
 */
/** One collapsed reasoning summary line: `✦ Thinking · N words`. */
function thinkingSummaryLine(reasoning: string, palette: Palette): string {
  const words = reasoning.trim().split(/\s+/u).filter(word => word !== '').length
  return palette.dim(`✦ Thinking · ${words} word${words === 1 ? '' : 's'}`)
}
...
function assistantMessageChildren(
  ...
  if (showsReasoning) {
    if (collapseThinking && !thinkingExpanded) {
      children.push(new Text(thinkingSummaryLine(reasoning, palette), 0, 0))
    } else {
      children.push(
...
export class StreamingAssistantComponent extends Container {
  ...
  constructor(
    ...
    private readonly collapseThinking: boolean,
    private readonly palette: Palette,
  ...
  /**
   * Expand or collapse this step's reasoning block. The collapsed form is one
   * summary line; expanded renders the reasoning inline.
   * @param expanded - Whether the reasoning block renders inline.
   */
  ...
  /** Toggle this step's reasoning block between the summary line and inline rendering. */
  toggleThinking(): void {
    this.setThinkingExpanded(!this.thinkingExpanded)
  }

  /**
   * Whether this step carries a reasoning block to fold.
   * @returns `true` when reasoning content exists regardless of visibility.
   */
  hasThinking(): boolean {
    return textBlocks(this.presentedContent(), 'reasoning').trim() !== ''
  }
...
export class ToolCardComponent extends CachedCardComponent {
  ...
  /**
   * The card's complete body for the full-screen detail pager: the same rows
   * as the expanded card, independent of the transcript's current visibility.
   * @param width - Render width the rows are wrapped to.
   * @returns the expanded card's rows.
   */
  detailLines(width: number): string[] {
    const saved = this.visibility
    this.visibility = 'expanded'
    this.dropLines()
    const lines = this.render(width)
    this.visibility = saved
    this.dropLines()
    return lines
  }
