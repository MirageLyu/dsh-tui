...
interface FakeAgent extends Agent {
  ...
  sentTargets: (InboxTarget | undefined)[]
...
export async function createTuiTestHarness<TerminalType extends Terminal, Exit extends (code: number) => void>(
  ...
  const sentTargets: (InboxTarget | undefined)[] = []
  ...
  const controller = createTuiChat(ctx, Object.assign({
    ...
    sessionId,
    // Reasoning renders inline in the legacy suites; the collapse behavior has
    // its own focused tests opting in through options.config.
