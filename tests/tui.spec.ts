  ...
  it('defaults every direct-call TUI option', () => {
    expect(resolveTuiConfig(undefined)).toEqual({
      ...
      theme: {
        ...
        scheme: undefined,
        truecolor: false,
        leftPrompt: '${cwd}${git/worktree}${permission}${plan}${model}${token_meter/cache_hit_rate}${context}',
        rightPrompt: '${queued}',
    ...
    expect(resolveTuiConfig({
      ...
      theme: { color: false, scheme: 'light', truecolor: true },
      title: 'DSH',
    })).toEqual({
      showReasoning: false,
      collapseThinking: false,
...
describe('pi-tui chat lifecycle and transcript', () => {
  ...
  it('retracts a failed live stream and renders its durable retry status', async () => {
    ...
    result.session.append('step/end', { turn: 1, step: 1 })
    result.session.append('llm/retry', {
      retryId: RetryId('retry-retract-1'),
      turn: 1,
      step: 1,
      provider: 'mock',
      mode: 'normal',
    ...
    result.session.append('llm/retry', {
      retryId: RetryId('retry-retract-2'),
      turn: 1,
      step: 2,
      provider: 'mock',
      mode: 'normal',
    ...
    result.session.append('llm/retry', {
      retryId: RetryId('retry-retract-always'),
      turn: 1,
      step: 3,
      provider: 'mock',
      mode: 'always',
  ...
  it('badges queued steering on the prompt context and clears it as each drains', async () => {
    ...
    const drainSteering = (text: string): void => {
      ...
      if (id !== undefined) {
        ...
        agentEvents(result.ctx, result.agent).emit('agent/inbox/claimed', { message, turn: 1 })
        // The loop logs a claimed steering message as a user/message, which
        // re-renders the transcript and shifts the prompt line back into view.
        result.session.append('user/message', message, { surfaceOp: 'append' })
      }
    }

    // An inbox claim for a different agent never touches this status line.
    ...
    // Another agent's discard, and one naming no pending id, leave the badge alone.
    agentEvents(result.ctx, other).emit('agent/inbox/discarded', { message: discarded[0]! })
  ...
  it('shows a live standalone compaction in the fixed status area', async () => {
    ...
    result.session.append('compaction/start', { compactionId: CompactionId('compaction-1'), turn: null })
    clock = 1_000
    result.terminal.output = ''
  ...
  it('ignores a numbered compaction bracket while the status line is idle', async () => {
    ...
    result.session.append('compaction/start', { compactionId: CompactionId('compaction-1'), turn: 1 })
    await tick()
  ...
  it('fades a closed standalone compaction back to the plain caret', async () => {
    ...
    result.session.append('compaction/start', { compactionId: CompactionId('compaction-1'), turn: null })
    await tick()
    result.session.append('compaction/end', { compactionId: CompactionId('compaction-1'), turn: null })
    await tick()
  ...
  it('reports a failed standalone compaction when its live bracket closes', async () => {
    ...
    result.session.append('compaction/start', { compactionId: CompactionId('compaction-1'), turn: null })
    result.terminal.output = ''
    result.session.append('compaction/end', { compactionId: CompactionId('compaction-1'), turn: null, error: 'summary failed' })
    await tick()
  ...
  it('does not show compaction progress for a resumed orphaned start', async () => {
    const result = await setup({
      ...
      now: () => 1_000,
      beforeMount(session) {
        session.append('compaction/start', { compactionId: CompactionId('compaction-1'), turn: null })
      },
    })
  ...
  it('combines session autocomplete with files and prepares send/steer references asynchronously', async () => {
    ...
    // Idle: the snapshot rides the prompt's admission (appended to the entered
    // messages), not a separate pre-admission inject.
    expect(result.agent.injected).toHaveLength(0)
    const preStep = () => agentEvents(result.ctx, result.agent).waterfall(
      'agent/pre-step',
      { messages: result.agent.sentMessages, turn: 1, step: 1, signal: new AbortController().signal },
      () => Promise.resolve({ kind: 'enter' as const, messages: [...result.agent.sentMessages] }),
    )
    const decision = await preStep()
    expect(decision.kind).toBe('enter')
    expect(decision.kind === 'enter' && decision.messages.at(-1)?.source)
    ...
    const replay = await preStep()
    expect(replay.kind === 'enter' && replay.messages.some(message => message.source.kind === 'session-reference')).toBe(false)
  ...
  it('releases the reference-admission wrapper on the ordinary allowed path', async () => {
    ...
    // Each wrapper releases on its own identified message's entered admission.
    // Running each prompt's pre-step waterfall detaches its wrapper.
    for (const sent of result.agent.sentMessages) {
      await agentEvents(result.ctx, result.agent).waterfall(
        'agent/pre-step',
        { messages: [sent], turn: 1, step: 1, signal: new AbortController().signal },
        () => Promise.resolve({ kind: 'enter' as const, messages: [sent] }),
      )
    }
    ...
    agentEvents(result.ctx, result.agent).emit('agent/inbox/discarded', { message: result.agent.sentMessages[0]! })
    ...
    expect(unrelated.kind === 'enter' && unrelated.messages.some(message => message.source.kind === 'session-reference')).toBe(false)
  ...
  it('releases the reference wrapper when enqueue synchronously discards before followup returns', async () => {
    ...
    // Real send() publishes its already identified snapshot, then an inbox
    ...
    result.agent.followup = (input) => {
      ...
      agentEvents(result.ctx, foreign).emit('agent/inbox/inserted', { message })
      agentEvents(result.ctx, result.agent).emit('agent/inbox/inserted', { message })
      agentEvents(result.ctx, result.agent).emit('agent/inbox/discarded', { message })
      return message.id
    }
  ...
  it('discards the reference snapshot with its blocked or cancelled prompt', async () => {
    ...
    // A downstream admission hook rejects the prompt: the attached snapshot
    // must be discarded with it, not stranded for the next prompt.
    let blockPrompts = true
    result.ctx.on('agent/pre-step', async (_payload, next) =>
    ...
    expect(passthrough.kind === 'enter' && passthrough.messages.some(message => message.source.kind === 'session-reference')).toBe(false)
    // A foreign agent's discard leaves the wrapper armed.
    const foreign = { ...result.agent, id: SessionId('foreign') } as unknown as Agent
    agentEvents(result.ctx, foreign).emit('agent/inbox/discarded', { message: result.agent.sentMessages.at(-1)! })
    ...
    agentEvents(result.ctx, result.agent).emit('agent/inbox/discarded', { message: result.agent.sentMessages.at(-1)! })
    await tick()
    ...
    const afterDiscard = await agentEvents(result.ctx, result.agent).waterfall(
      'agent/pre-step',
      { messages: [result.agent.sentMessages.at(-1)!], turn: 1, step: 1, signal: new AbortController().signal },
      () => Promise.resolve({ kind: 'enter' as const, messages: [result.agent.sentMessages.at(-1)!] }),
    )
    expect(afterDiscard.kind === 'enter' && afterDiscard.messages.some(message => message.source.kind === 'session-reference')).toBe(false)
    await dispose(result)
  })
  ...
  it('falls back cleanly for non-session, empty, failed, and superseded autocomplete requests', async () => {
    ...
    const sessionReferences = result.ctx.get('sessionReferenceResolver')!
    const originalListCandidates = sessionReferences.listCandidates.bind(sessionReferences)
    const listCandidates = vi.spyOn(sessionReferences, 'listCandidates')
  ...
  it('clears a retyped successful mention and aborts pending preparation on disposal', async () => {
    ...
    const prepare = vi.spyOn(result.ctx.get('sessionReferenceResolver')!, 'prepare').mockImplementation(
    ...
    const latePrepare = vi.spyOn(lateSuccess.ctx.get('sessionReferenceResolver')!, 'prepare').mockImplementation(
      (_agent, content) => new Promise((resolve) => {
    ...
    const result = await setup({
      beforeMount(session) {
        ...
        session.append('user/message', createUserMessage({
          ...
          source: compactCheckpointSource(CompactionId('checkpoint-2')),
        }), {
          surfaceOp: { op: 'replace', start: nodes[0] as number, end: nodes.at(-1) as number },
          sourceEventSeqs: nodes,
        })
...
describe('terminal mounting', () => {
  /** Build a minimal live Agent for mount-startup tests that never drives a turn. */
  const fakeAgent = (ctx: Context, session: Session, status: AgentStatus = 'idle'): Agent => ({
    id: session.id,
    options: {},
    session,
    status,
  ...
  it('starts immediately when the configured agent already exists', async () => {
    ...
    ctx.agents.register(fakeAgent(ctx, session))
    const terminal = new FakeTerminal()
  ...
  it('waits for its configured agent before starting the TUI', async () => {
    ...
    ctx.agents.register(fakeAgent(ctx, otherSession))
    expect(terminal.started).toBe(0)
  ...
  it('rolls back providers, listeners, and terminal state when startup fails', async () => {
    ...
    ctx.agents.register(fakeAgent(ctx, session, 'running'))
    const terminal = new FakeTerminal()
...
describe('thinking collapse and theme switching', () => {
  const thought = createUserMessage({
    content: [{ type: 'text', text: 'prompt' }],
    source: { kind: 'user' },
  })
  const withReasoning = (session: ReturnType<typeof SessionStore.prototype.create>) => {
    session.append('user/message', thought, { surfaceOp: 'append' })
    session.append('assistant/message', {
      turn: 1,
      step: 1,
      message: createMessage({
        role: 'assistant',
        content: [
          { type: 'reasoning', text: 'restored thought and more words here' },
  ...
  it('collapses reasoning into a summary line by default and expands with Ctrl+T', async () => {
    const result = await setup({
      omitInitialLifecycle: true,
      config: { collapseThinking: true },
      beforeMount: (session) => { withReasoning(session) },
    })
    await tick()
    // The terminal output accumulates frames, so assertions count occurrences
    // of each presentation rather than expecting the other one to vanish.
    expect(result.terminal.output).toContain('✦ Thinking · 6 words')
    expect(result.terminal.output.split('restored thought and more words here')).toHaveLength(1)
    result.terminal.send('\x14') // Ctrl+T expands inline
    await tick()
    expect(result.terminal.output.split('restored thought and more words here')).toHaveLength(2)
    result.terminal.send('\x14')
    await tick()
    // The collapsed summary appears again; no further inline copy is emitted.
    expect(result.terminal.output.split('✦ Thinking · 6 words').length).toBeGreaterThanOrEqual(2)
    expect(result.terminal.output.split('restored thought and more words here')).toHaveLength(2)
    await disposeTuiTestHarness(result)
  })

  it('renders reasoning inline when collapseThinking is off', async () => {
    const result = await setup({
      omitInitialLifecycle: true,
      config: { collapseThinking: false },
      beforeMount: (session) => { withReasoning(session) },
    })
    await tick()
    expect(result.terminal.output).toContain('restored thought and more words here')
    await disposeTuiTestHarness(result)
  })

  it('/theme switches the scheme and reports it', async () => {
    const result = await setup({ omitInitialLifecycle: true })
    result.terminal.send('/theme light')
    result.terminal.send('\r')
    await tick()
    expect(result.terminal.output).toContain('scheme light')
    result.terminal.send('/theme')
    result.terminal.send('\r')
    await tick()
    expect(result.terminal.output).toContain('current scheme light')
    await disposeTuiTestHarness(result)
  })

  it('Ctrl+Enter opens the full-screen tool detail pager and Esc closes it', async () => {
    const result = await setup({
      omitInitialLifecycle: true,
      config: { collapseThinking: false },
      tools: {
        visual: {
          name: 'visual',
          description: 'pager fixture',
          parameters: {},
          output: { schema: { type: 'null' }, render: () => [] },
          execute: () => Promise.resolve([]),
          presentCall: () => ({ card: 'generic' as const, title: 'Visual', rawInput: { x: 1 } }),
          presentResult: () => ({ card: 'generic' as const, content: [{ type: 'text' as const, text: 'line one\nline two\nline three' }] }),
        },
      },
    })
    ...
    result.session.append('tool/call', {
      turn: 1, step: 1, callId: CallId('visual-call'), name: 'visual', arguments: '{"x":1}',
    })
    result.session.append('tool/result', {
      turn: 1, step: 1,
      message: createToolResultMessage({
        callId: CallId('visual-call'),
        content: [{ type: 'text', text: 'line one\nline two\nline three' }],
        isError: false,
      }),
    }, { surfaceOp: 'append' })
    await tick()
    result.terminal.send('\x06') // Ctrl+F
    await tick()
    expect(result.terminal.output).toContain('Detail')
    expect(result.terminal.output).toContain('line three')
    result.terminal.send('\x1B')
    await tick()
    // The accumulated stream keeps the opened frame; closing adds no more.
    expect(result.terminal.output.split('Detail')).toHaveLength(2)
    await disposeTuiTestHarness(result)
  })
})
