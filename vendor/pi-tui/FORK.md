...
under its fork name (`@miragelyu/pi-tui`) via the `workspace:*` protocol, which the release rewrites to the concrete
version, so every install resolves to the patched fork.
